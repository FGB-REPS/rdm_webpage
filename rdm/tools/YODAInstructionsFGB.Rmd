...
* YODA has a data [publishing](https://yoda.vu.nl/site/getting-started/publishing.html){target="_blank"} option that assigns a permanent identifier to the published materials. This will, at a minimum, openly publish the structured metadata about the dataset; whether the data itself is openly available depends on the chosen data access option. There must be a [PURE registration for any datasets used in a research article](../archiving/ArchivingGuidelinesFull.html#purereg){target="_blank"}, even if the data are not openly available, and YODA offers the easiest method to achieve this requirement. Published metadata in YODA can be easily imported into PURE. **You still need to register your datasets in PURE**, but the [import option](https://libguides.vu.nl/pure-managing-publications/add-datasets){target="_blank"} greatly facilitates the process.
...
* YODA has some other technical limitations, including:
  ...
  + YODA does not use a [desktop client](https://wiki.surfnet.nl/display/RDRIVE/ownCloud+desktop+client){target="_blank"} to sync data to a local folder on your computer. 
    + There are several [ways](https://rdm.vu.nl/manuals/yoda/data_access/introduction.html){target="_blank"} to access data in YODA; if you are working with particularly large files or data that should not be copied locally, WebDAV or rclone are your best options for connecting to your YODA drive.
    + If you use an [option](https://rdm.vu.nl/manuals/yoda/data_access/introduction.html){target="_blank"} that downloads data to your device, e.g. Cyberduck, make sure to review the [data protection](#protection) section on how to handle locally copied data. 
...
<!--Per May 2024 and after discussion with colleagues, have opted to update that YODA can be used for PURE registration as the process is mostly complete. Will discuss in section below -->
<!-- Per Nov 2025 learned that auto ingest of YODA data into PURE is not possible. Researchers have to make a registration in PURE, but they can import YODA data rather than filling everything in -->
...
* Consider using [WebDAV or rclone](https://rdm.vu.nl/manuals/yoda/data_access/introduction.html){target="_blank"} to connect to your YODA drive so that you can avoid the necessity of copying data locally.
* If you opt to upload and download data in YODA, e.g via [Cyberduck](https://yoda.vu.nl/site/getting-started/yoda-disk-cyberduck.html){target="_blank"}, remember that these files will be copied locally to your device. When working with "Red" data this means:
...
* Consider using [WebDAV or rclone](https://rdm.vu.nl/manuals/yoda/data_access/introduction.html){target="_blank"} to connect to your YODA drive so that you can avoid the necessity of copying data locally
* If you opt to upload and download data in YODA, e.g via [Cyberduck](https://yoda.vu.nl/site/getting-started/yoda-disk-cyberduck.html){target="_blank"}, remember that these files will be copied locally to your device. When working with "Orange" data this means:
    * **It is imperative that you have [full-disk encryption](../../Security/Encryption.html#fde){target="_blank"} activated on your computer**
    * You must properly manage where the files are copied to on your computer, so that you can keep track of them. When setting up your connection to YODA with Cyberduck, you must define to which directory (folder location) the downloaded files should be copied to. This way you can better manage your local files, prevent accidental overwriting of these files and, most importantly, allow you to easily [delete](../../Security/SecurityBasics.html#removal){target="_blank"} all of the files in this location when you no longer need them stored locally.
...
* Consider using [WebDAV or rclone](https://rdm.vu.nl/manuals/yoda/data_access/introduction.html){target="_blank"} to connect to your YODA drive so that you can avoid the necessity of copying data locally
* If you opt to upload and download data in YODA, e.g via [Cyberduck](https://yoda.vu.nl/site/getting-started/yoda-disk-cyberduck.html){target="_blank"}, remember that these files will be copied locally to your device. When working with "Yellow" data this means:
    * **It is imperative that you have [full-disk encryption](../../Security/Encryption.html#fde){target="_blank"} activated on your computer**
    * You must properly manage where the files are copied to on your computer, so that you can keep track of them. When setting up your connection to YODA with Cyberduck, you must define to which directory (folder location) the downloaded files should be copied to. This way you can better manage your local files, prevent accidental overwriting of these files and, most importantly, allow you to easily [delete](../../Security/SecurityBasics.html#removal){target="_blank"} all of the files in this location when you no longer need them stored locally.
    * You must ensure that even read-only users are aware of these rules. Even though they are not able to modify any data in YODA, they can still download data. You must ensure that all read-only users will properly manage data that has been stored locally and that they remove it from their computer as soon as it is no longer required.
...
* Consider using [WebDAV or rclone](https://rdm.vu.nl/manuals/yoda/data_access/introduction.html){target="_blank"} to connect to your YODA drive so that you can avoid the necessity of copying data locally
* If you opt to upload and download data in YODA, e.g via [Cyberduck](https://yoda.vu.nl/site/getting-started/yoda-disk-cyberduck.html){target="_blank"}, remember that these files will be copied locally to your device. When working with "Green" data this means:
    * You must properly manage where the files are copied to on your computer, so that you can keep track of them. When setting up your connection to YODA with Cyberduck, you must define to which directory (folder location) the downloaded files should be copied to. This way you can better manage your local files, prevent accidental overwriting of these files and, most importantly, allow you to easily [delete](../../Security/SecurityBasics.html#removal){target="_blank"} all of the files in this location when you no longer need them stored locally.
    * You must ensure that even read-only users are aware of these rules. Even though they are not able to modify any data in YODA, they can still download data. You must ensure that all read-only users will properly manage data that has been stored locally and that they remove it from their computer as soon as it is no longer required.
...
* Consider using [WebDAV or rclone](https://rdm.vu.nl/manuals/yoda/data_access/introduction.html){target="_blank"} to connect to your YODA drive so that you can avoid the necessity of copying data locally
* If you opt to upload and download data in YODA, e.g via [Cyberduck](https://yoda.vu.nl/site/getting-started/yoda-disk-cyberduck.html){target="_blank"}, remember that these files will be copied locally to your device. You should therefore wise properly manage where the files are copied to on your computer, so that you can keep track of them. When setting up your connection to YODA with Cyberduck, you should define to which directory (folder location) the downloaded files should be copied to. By managing your local files well, you can prevent accidental overwriting of these local files.
...
As a starting point, you can use this [YODA archiving checklist](../../Images/meta_archiving_guide_yoda_v2.1.docx) to make sure you've done all the necessary steps to archive data in YODA. The following information goes into even further detail if you need it.
...
* Once your research is complete and the associated data and materials (called the "YODA data package") need to be archived, you should archive the data package in the YODA ["vault"](https://yoda.vu.nl/site/getting-started/archiving.html){target="_blank"}. Information on _when_ you should archive your research materials, as well as _which materials_ should be archived with the data, can be found in the [FGB Archiving Guidelines](../archiving/ArchivingGuidelinesSummary.html){target="_blank"}
  ...
  * When you are completing your metadata, you also need to select an appropriate license: "restricted access" and "closed" data require a custom license, and "open-freely retrievable" data need to be licensed with one of the options listed in the metadata form
    ...
    * You can find more information on custom licenses on the [VU YODA manual](https://yoda.vu.nl/site/getting-started/selecting-license.html){target="_blank"}
      * If you need guidance on which custom license to use or need help with filling in the available templates, contact the [FGB Research Data Stewards](mailto:research.data.fgb@vu.nl)
      * **NB:** You need to save a copy of the custom license in a different folder from your restricted access/closed data because, if you don't, third parties won't be able to review the license without first requesting access (defeating the purpose of the license). FGB recommends publishing a copy of the license on [OSF](https://osf.io/abwzm/){target="_blank"} and cross-referencing to this OSF publication in your YODA metadata in the "related data package" field. You can also consider putting all of the research documentation that can be openly shared with others (such as the [README](../archiving/ArchivingGuidelinesFull.html#subsection9){target="_blank"}) on OSF and cross-referencing to this OSF publication in the YODA metadata. A third option is to use separate a YODA folder, one that is categorized as "open-freely retrievable", for the license and all of the documentation that can be openly published, and then cross-referencing to this in the YODA metadata for the closed/restricted access data.
...
* You can [publish](https://yoda.vu.nl/site/getting-started/publishing.html){target="_blank"} any materials that have been submitted to the vault. Publishing is required when the data have been used for a research article. **This does not mean that the data are made openly available as long as you followed the guidance in the section on [Data Protection](#protection).** Most FGB datasets will be ["restricted access" or "closed"](#safePublishing), resulting in the metadata being openly published (and therefore visible to the world), which gives the dataset a DOI permanent identifier, but the dataset itself will not be openly available.
  ...
  * If you publish in YODA, the metadata from that publication can be imported into [PURE](https://vu.nl/en/employee/research-data-support/vu-research-portal-pure){target="_blank"} when you register your dataset. PURE registration of datasets is a [requirement](../archiving/ArchivingGuidelinesFull.html#section4){target="_blank"} whenever data are archived. 
