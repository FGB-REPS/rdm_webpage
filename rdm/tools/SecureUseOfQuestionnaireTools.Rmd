...
VU Amsterdam has a university-wide contract and processing agreement with Qualtrics. Any VU employee can sign into Qualtrics using their VU single sign-on details. 
FGB also has a faculty-wide contract and processing agreement with Survalyzer. Access to Survalyzer can be requested by contacting the [Faculty Survey Administrator](mailto:qualtrics.fgb@vu.nl). There are a limited number of workspaces available, however, so it's possible the Survey Admin will have to reject your request for access.

### Why are there two questionnaire tools?

Survalyzer was introduced originally because the faculty was able to set up a [processing agreement](../../legal/GDPRBasics.html#GDPRroles){target="_blank"} with Survalyzer's provider. At the time, the VU was unable to get a processing agreement in place with Qualtrics, which limited how we could use Qualtrics in the faculty, particularly for very sensitive surveys. Both options now have processing agreements, but still require some enhancements to their security measures to allow for the highest risk data to be collected using just one of the options. Essentially this means that if the content of your survey is very sensitive and/or your research subjects are very vulnerable, it's generally advised to use Survalyzer for collecting more directly identifying information (e.g. names, contact information, digital consent) and Qualtrics for collecting the actual research data. How you can achieve this is explained further [below in the section on Secure Use](#qToolsSec).


If your data are not high risk ([privacy category yellow, green or blue](../../Security/PrivacyRisks.html){target="_blank"}), you can use either Qualtrics or Survalyzer for your entire data collection, whichever better suits your purposes. To help you pick an option, you can review the technical specifications of each below:
...
| **Data storage** | &bull; Data are stored on EU servers <br> &bull; Data are backed up and stored for 90 days; any data deletions are only permanent after 90 days have elapsed | &bull; Data are stored on EU servers <br> &bull; Data are backed up for 90 days. When data are deleted they are flagged for 90 days and then deleted, and a back-up of this data is saved for a further 90 days, i.e. data deletions are onoy permanent after 180 days have elapsed |
